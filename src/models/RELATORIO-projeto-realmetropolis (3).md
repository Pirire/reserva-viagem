...
## 11. ATUALIZAÇÃO — SESSÃO 3 (feature do código + despacho)

### 11.1 FEITO nesta sessão (correções aplicadas e a funcionar)
- ✅ **"Sessão necessária"** corrigido (requireClienteOuParceiro). Commit f2f74af.
- ✅ **Contacto repetido** — valida e avisa antes de criar (evita ShareTrip órfão). Commit f2f74af.
- ✅ **OTP na criação** (fluxo de vários participantes). Commit f2f74af.
- ✅ **Código de confirmação antes de "chamar o motorista"** — FEATURE NOVA, funciona ponta a ponta:
  - `ShareInvite.js`: campos novos `prontoOtpHash` / `prontoOtpExpira`
  - `partilha.routes.js`: gera código no pagamento (confirmar-pagamento), inclui no SMS+email,
    valida no `/evento/estou-pronto` antes de despachar
  - `rm-share.js`: campo de código no ecrã "chamar motorista" + envia no body
  - Testado: pagamento → código no email → código certo despacha, errado recusa. ✅

### 11.2 BUG ATUAL A RESOLVER (prioridade nº 1)
**O motorista NÃO recebe nada — nada aparece no painel do motorista** quando o hóspede
carrega "ESTOU PRONTO" (com código válido). O despacho é acionado (`criarEDespacharViagem`
em partilha.routes.js ~2686) mas o motorista não vê o pedido.
- A investigar: `criarEDespacharViagem.service.js` (não visto ainda) e `dispatch.events.js`.
- Relembrar o desalinhamento de estado (secção 3.4): `assigned` (inglês) vs `atribuida`
  (português) entre reservas.routes.js e partilha.routes.js.
- Este é o ALICERCE — tem de funcionar antes de construir o modelo "mesmo veículo".

### 11.3 MODELO ATUAL confirmado: viagens INDEPENDENTES
- `reserva-simples/criar` grava `mesmoVeiculo: false` (linha ~1697).
- Cada participante tem o seu `destinoParticipante` próprio.
- Ao "ESTOU PRONTO", cria uma Trip SÓ para esse participante, com o destino dele.
- Ou seja: cada pessoa paga a sua parte, chama o seu motorista, viaja para o seu destino,
  INDEPENDENTEMENTE dos outros. Um pagar/despachar NÃO afeta os outros. Ninguém "perde"
  a viagem por causa de outro. (Respondeu à dúvida do utilizador.)

### 11.4 FEATURES FUTURAS pedidas pelo utilizador (Fase 2 — sessões dedicadas)
**A) Opção "Todos no mesmo veículo"**
- Checkbox que aparece ao selecionar "+ Convidado" (o botão renomeado — ver B).
- Se marcada → todos no MESMO carro, uma só viagem, um só motorista (modelo NOVO, não existe).
- Se não marcada → cada um no seu (modelo atual).
- O schema ShareTrip JÁ tem o campo `mesmoVeiculo` — falta a UI e, sobretudo, o fluxo de
  despacho de grupo (uma Trip para vários, recolha comum, paragens/destino).

**B) Renomear** "+ Convidar mais pessoas" → "**+ Convidado**" (trivial, UI só).

**C) Regra de pagamento no "mesmo veículo" (com cronómetro):**
- Todos recebem pedido de pagamento.
- Contagem DECRESCENTE para pagar.
- Quem paga a tempo → fica na viagem.
- Quem NÃO paga a tempo → excluído automaticamente.
- Ao fim do tempo → despacho feito só com os que pagaram, um só veículo/motorista.

**D) App Android** (utilizador já tem Android Studio):
- Para o caso "mesmo veículo": mostra lista de quem já pagou, com cronómetro decrescente,
  a lembrar que tem de pagar senão a sua parte é cancelada automaticamente.

### 11.5 ORDEM RECOMENDADA
1. **PRIMEIRO:** resolver o bug do despacho (11.2) — motorista tem de receber o pedido.
2. Depois: UI pequena — renomear "+ Convidado" + checkbox visual "mesmo veículo" (guardar mesmoVeiculo).
3. Fase 2 (dedicada): fluxo despacho "mesmo veículo" + cronómetro/exclusão (C) + app Android (D).

---

## 11. ATUALIZAÇÃO — SESSÃO 3 (dia seguinte, cabeça fresca)

### 11.1 CORREÇÕES FEITAS E TRANCADAS NO GIT
Todas testadas a funcionar e com commit+push:
- **`3fecf11`** — "Sessão necessária": rota `/reserva-simples/criar` usava `requireCliente`
  (sessão de cliente) mas o hotel-dashboard dá sessão de PARCEIRO (cookie `rm_parceiro_token`,
  typ `parceiro`). Criado `requireClienteOuParceiro` em `utils/clienteAuth.js` que aceita ambos.
- **`f2f74af`** — Validação de contacto repetido (ShareInvite tem índice único
  {shareId, contactoNorm}; dois participantes com mesmo contacto falhavam em silêncio).
  Agora avisa "CONTACTO_REPETIDO" antes de criar nada. + OTP na criação.
- **`4532d60`** — FEATURE código antes de chamar motorista (backend + frontend, testada
  ponta a ponta): gera código no pagamento (`prontoOtpHash`/`prontoOtpExpira` no ShareInvite),
  envia no SMS/email pós-pagamento, valida em `/evento/estou-pronto`, campo de código no
  ecrã (rm-share.js). FUNCIONA para o hóspede principal.

### 11.2 PARTE A APLICADA (caixa "mesmo veículo" + renomear) — testar/commit pendente
Ficheiros entregues (rm-core.js + hotel-dashboard.html):
- Botão "+ Convidar mais pessoas" renomeado para **"+ Convidado"**
- Caixa **"Todos no mesmo veículo (viagem partilhada)"** aparece ao adicionar convidado
- Envia `mesmoVeiculo` (bool) ao backend em `/reserva-simples/criar`
- Versões dos scripts subidas para matar cache: rm-core?v=3, rm-share?v=22, rm-events?v=10
- NOTA: Parte A só MOSTRA a caixa e GUARDA o valor. NÃO muda ainda o comportamento
  despacho/preço (isso é a "ligação ao Partilhar Viagem", ver 11.4).

### 11.3 BUGS ABERTOS (a resolver na próxima sessão)
**BUG A — código do CONVIDADO diz "código incorreto"** (o do principal funciona).
Hipóteses: (1) convidado não completou o pagamento → `prontoOtpHash` nunca gerado;
(2) destino do convidado não gravou e o fluxo dele partiu. PRECISA dos dados reais.
→ Criado script **`verInvites.js`** (em outputs) que mostra por invite: pago, amountDue,
  destinoParticipante (addr/lat/lng), prontoOtpHash. Correr `node verInvites.js` e ler os
  campos do convidado para saber a causa.

**BUG B — campo "destino deste participante" não grava / aparece "?" na partilha.**
Em `rm-core.js`, `lerParticipantesExtra()` IGNORA a linha se `destinoInput.dataset.lat`
estiver vazio (só preenchido ao clicar no autocomplete). O hóspede PRINCIPAL tem
geocodificação automática de fallback; os CONVIDADOS não. Utilizador diz ter selecionado
o autocomplete em todos — por isso confirmar com verInvites.js se o destino gravou mesmo.
Correção provável: dar aos convidados a mesma geocodificação automática do principal.

**BUG C (o mais crítico, de sessão anterior) — "o motorista não recebeu nada".**
Ao chamar motorista, nada aparece no painel do motorista. Afeta TUDO. Provável desalinhamento
de estado: `reservas.routes.js` procura `trip.status === "assigned"` (inglês) vs
`partilha.routes.js` `["atribuida","em_viagem","aceite","confirmada"]` (português).
PRECISA de ver `dispatch.events.js` (ainda não visto) para saber que valor o dispatch grava.

### 11.4 FEATURE GRANDE PLANEADA — "mesmo veículo" ligado ao Partilhar Viagem
DECISÃO tomada: quando "mesmo veículo" marcado, LIGAR ao fluxo Partilhar Viagem existente
(não duplicar). O que já existe e vamos reutilizar:
- Categorias de grupo COM preço (quote_routes.js): grupo6=1.20, grupo8=1.35, grupo17=1.60 €/km
  (+ economica 0.85, confort 1.05, executive 1.35, luxury 1.75). Nomes internos: grupo6/grupo8/grupo17.
- Tabela passageiros→categoria (decidida pelo utilizador):
  1-4 = normal escolhida | 5-6 = grupo6 | 7-8 = grupo8 | 9-16 = grupo17 | 17+ = recusar (máx 16)
- Cálculo JUSTO multi-paragem já existe na rota **`/calc`** (partilha.routes.js ~1373-1560):
  calcula rota recolha→paragens→destino mais longe, preço total, e divide por ratio de km
  (quem anda mais paga mais: `ratio = km_dele / km_total`, `amountDue = total * ratio`).
- Recolha comum + SMS a cada participante a pedir/confirmar destino já existe.
Modelo do "mesmo veículo": recolha comum (todos do mesmo sítio), cada um indica o seu destino
por SMS (ou aceita o sugestivo do organizador), carro faz rota multi-paragem até ao destino
mais distante, custo dividido de forma justa por km.

PASSOS para a próxima sessão (feature mesmo veículo):
1. Contar passageiros no rm-core → validar limite 16 → escolher categoria grupo6/8/17
2. Quando mesmoVeiculo=true, encaminhar para o fluxo Partilhar Viagem (rota /calc para o preço)
   em vez de criar viagens independentes
3. Despacho como UMA viagem (não uma por pessoa)
Ficheiros relevantes já recolhidos: convidado_routes.js, partilha.routes.js,
partilhaInvite.helpers.js (versão alternativa de criação de convites — cuidado com duplicação).

### 11.5 Estado do git no fim da sessão 3
Último commit trancado: **`4532d60`** (feature código chamar motorista).
Parte A (caixa mesmo veículo) entregue mas AINDA NÃO testada/committed pelo utilizador.

### 11.6 Ordem sugerida para a próxima sessão
1. Testar + commit da Parte A (caixa "mesmo veículo" + "+ Convidado")
2. Correr `verInvites.js` → diagnosticar BUG A (código convidado) e BUG B (destino não grava)
3. Corrigir BUG C (despacho — motorista não recebe) — ver dispatch.events.js. É o mais crítico.
4. Construir a feature "mesmo veículo" ligada ao Partilhar Viagem (11.4)

### 11.7 Ficheiros-chave vistos nesta sessão (para referência rápida)
- utils/clienteAuth.js — auth de cliente/hotel (requireCliente, requireClienteOuParceiro)
- routes/parceiroInvite.routes.js — cria sessão parceiro (rm_parceiro_token, typ parceiro)
- routes/partilha.routes.js — reserva-simples/criar (~1641), /calc (~1373), evento/estou-pronto
  (~2609), evento/confirmar-pagamento (~2299)
- models/ShareInvite.js — schema (índice único {shareId,contactoNorm}, campos otp + prontoOtp)
- routes/quote_routes.js — categorias e preços (incl. grupos)
- js/rm-core.js — reservarViagem, lerParticipantesExtra, adicionarParticipanteExtra
- js/rm-share.js — ecrã estou-pronto (campo de código), mapas de grupo
- middlewares: authAdmin.js (admin_token), authGestorOrPartner.js (colab_token)

---

## 12. FEATURE "EU PAGO" — DESENHO COMPLETO (decidido, pronto a construir)

### Conceito
Permite escolher QUEM paga por QUEM numa reserva com vários participantes. Flexível:
o hotel pode só organizar sem pagar; qualquer participante pode assumir o pagamento
de si + de outros.

### Regras (todas decididas pelo utilizador)
1. Cada participante tem um NÚMERO (Participante 1, 2, 3...).
2. Caixa "Eu pago" (ao lado da caixa "mesmo veículo") — ao marcar, mostra a lista dos
   números dos participantes em MULTI-SELEÇÃO.
3. Quem marca "Eu pago" seleciona quais paga (o seu + outros). Ex: participante 1 marca
   "pago a minha + a 2 + a 3".
4. PAGAMENTO: o pagador paga a SOMA ACUMULADA de todas as viagens que assumiu, num
   ÚNICO pagamento, na hora (recebe UM link com o valor total do que assumiu).
5. Quem é PAGO por outro → recebe SÓ o código de confirmação (sem passo de pagamento).
6. Quem NÃO é selecionado por ninguém → paga a sua própria parte (fluxo normal).
7. O HOTEL/organizador pode só organizar, sem pagar nada.
8. REGRA ANTI-CONFLITO: cada participante só pode ser pago por UM pagador. Primeiro a
   marcar "fica" com ele; mais ninguém o pode selecionar depois. Evita cobrança dupla.

### Lógica reutilizável que JÁ EXISTE (mas no sistema ANTIGO — Reserva)
`src/routes/convidado.routes.js` já tem a base:
- linha 223/292: `valorTotal` — soma das viagens
- linha 263: `pagadorTipo` ("remetente" | "convidado")
- linha 264: `pagadorConvidadoIdx` — qual convidado paga (por índice = o "número")
- linha 393/394: `pagador: "remetente", totalCobrado: valorTotal` — pagador cobre o total
- linha 402+: CASO "UM CONVIDADO PAGA" — cria Reserva pendente, envia ticket-link SÓ ao
  pagador, despacho arranca quando esse pagamento confirma
- usa modelo `Reserva` + `criarEDespacharViagem` + tokenTicket/codigoTicket
⚠️ ALERTA: está no modelo `Reserva` (ANTIGO). Tem de ser TRADUZIDO para `ShareInvite`
(NOVO) — trabalho delicado, com pagamentos. Fazer com cabeça fresca, passo a passo, git a cada passo.

### O que construir (quando for a altura)
FRONTEND (rm-core.js):
- Numerar cada participante (1, 2, 3...) visível no formulário
- Caixa "Eu pago" ao lado de "mesmo veículo"; ao marcar, mostra multi-seleção dos números
- Aplicar regra anti-conflito na UI (número já assumido por outro fica indisponível)
- Calcular e mostrar o valor acumulado do pagador em tempo real
- Enviar ao backend: para cada participante, quem é o seu pagador (ou "próprio")

BACKEND (traduzir de convidado.routes.js para ShareInvite/partilha.routes.js):
- Marcar cada ShareInvite com o seu pagador (quem cobre o quê)
- Para o pagador: gerar link com valor ACUMULADO (soma dos amountDue que assumiu)
- Para os cobertos: marcar como "pago por X", saltar pagamento, gerar só o código pronto
- Para os não-cobertos: fluxo normal (paga a sua parte)
- Validar a regra anti-conflito no servidor (não confiar só na UI)

### PRÉ-REQUISITOS antes de construir "Eu pago"
1. Resolver BUG C (despacho — motorista não recebe). Sem despacho, nada disto entrega viagem.
2. Idealmente consolidar/decidir o sistema, porque a lógica-fonte está no antigo (Reserva)
   e o destino é o novo (ShareInvite).
